...
class ArbolID3:
    ...
    def entropia(self, atributo, ycol):  # Calcula la entropía de un solo atributo
        ...
        for valor in dftrain[atributo].unique():
            ...
            for resultado in filasQueImportan[ycol].unique():
                ...
                proba2 = len(resultadosQueImportan.index) / \
                    len(filasQueImportan.index)
    ...
    # Selecciono el atributo con menor entropía que sea mayor a cero
    ...
    def entrenar(self, dftrain, ycol):
        # Creo una lista de los atributos (para pasarlos luego)
        self.atributos = list(dftrain.columns)
    ...
    # Encuentro los caminos que salen de cada nodo
    ...
    # Para cada renglón del dataset de prueba, realizo una predicción
    def realizarPrognosis(self, caso):
        ...
        for index, value in caso.items():
            if len(nodoActual.hijos) == 0:
                ...
                #predic =  ""
                predic = (nodoActual.padre.valor + "-" +
                          str(nodoActual.arista))if nodoActual.padre != None else ""
                # print(predic)
...
def pprint_tree(node, file=None, _prefix="", _last=True):
    print(_prefix, "`-" if _last else "|-", node.arista if node.arista !=
          None else "", "-> ", node.valor, sep="", file=file)
...
def crearArbolDiccionario(nodo):  # Creo un megadiccionario
    ...
    if nodo.arista != None:
        # if type(nodo.arista) is int else nodo.arista
...
# Conforme voy leyendo los diccionarios voy creando los nodos
